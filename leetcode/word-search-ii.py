...
class Solution:
    def findWords(self, board: List[List[str]], words: List[str]) -> List[str]:
        ...
        def dfs(x, y, currentWord, visited, node):
            ans = []
            if node.isWord: ans.append(currentWord)

            if x < 0 or x >= m or y < 0 or y >= m or (x, y) in visited:
                return ans

            visited.add((x, y))
            character = board[x][y]
            if character in node.children:
                ans += dfs(x + 1, y, currentWord + character, set(visited), node.children[character])
                ans += dfs(x - 1, y, currentWord + character, set(visited), node.children[character])
                ans += dfs(x, y + 1, currentWord + character, set(visited), node.children[character])
                ans += dfs(x, y - 1, currentWord + character, set(visited), node.children[character])
            return ans
        ...
        def bfs(x, y):
            ...
            if character in root.children:
                q.append((x, y, root.children[character], character, []))
            while q:
                # print(q)
                x, y, cur, currentStr, visited = q.popleft()
                # print('pop', currentStr)
                if cur.isWord:
                    # print('found', currentStr)
                    res.append(currentStr)
                visited.append((x, y))
                for dx, dy in directions:
                    xx, yy = x + dx, y + dy                    
                    if 0 <= xx < m and 0 <= yy < n and (xx, yy) not in visited:
                        # print('check', xx, yy, 0 <= xx < m, 0 <= yy < n, (xx, yy) not in visited, board[xx][yy], board[xx][yy] in cur.children, cur.children.keys())
                        if board[xx][yy] in cur.children:
                            # print(board[xx][yy])
                            q.append((xx, yy, cur.children[board[xx][yy]], currentStr + board[xx][yy], visited + [(xx, yy)]))
        for w in words:
            insert(w)
        ...
        for i in range(m):
            for j in range(n):
                # bfs(i, j)
                res += dfs(i, j, "", set(), root)
        return set(res)

board, words = [["o","a","a","n"],["e","t","a","e"],["i","h","k","r"],["i","f","l","v"]], ["oath","pea","eat","rain"] # [eat, oath]
board, words = [["a","b"],["c","d"]], ["abcb"] # []
board, words = [["o","a","b","n"],["o","t","a","e"],["a","h","k","r"],["a","f","l","v"]], ["oa", "oaa"] # ["oa", "oaa"]
board, words = [["o","a","a","n"],["e","t","a","e"],["i","h","k","r"],["i","f","l","v"]], ["oath","pea","eat","rain","hklf", "hf"] # ["oath","eat","hklf","hf"]
